    ...
    if (ParseStatusUtils.isSuccess(pstatus)) {
      if (pstatus.getMinorCode() == ParseStatusCodes.SUCCESS_REDIRECT) {
        ...
        try {
        	...
        	if(!content.equals("")) {
        		String city = page.getBaseUrl().toString();
        		city = city.substring(8, city.indexOf("."));
        		String title = page.getTitle().toString();
        		String basePath = "D:\\eclipse\\eclipse-workspace\\branch-2.2.1\\logs\\内容\\";
        		File f = new File(basePath+city);
        		if(!f.exists()) {
        			f.mkdirs();
        		}
        		String fileName = basePath+city+"\\"+title+".txt";
        		FileWriter writer = new FileWriter(fileName, true);   
        		writer.write(page.getBaseUrl().toString());
                writer.write("\r\n");
                writer.write("title:"+parse.getTitle());
                writer.write("\r\n");
                writer.write("内容:"+parse.getText());
                writer.write("\r\n");
                writer.close(); 
        	}
    	} catch (IOException e) {
    		 e.printStackTrace();
    		 System.out.println("写出内容错误...............................................");
    		 LOG.info("写出内容错误...............................................");
