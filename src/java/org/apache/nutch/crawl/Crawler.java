...
public class Crawler extends NutchTool implements Tool {
  ...
  public static StringBuffer getInfo(String[] args) {
	  StringBuffer info = new StringBuffer();
	  for (int i = 0; i < args.length; i++) {
	      if ("-threads".equals(args[i])) {
	    	  info.append("启动"+Integer.parseInt(args[i+1])+"个线程爬取数据,");
	    	  i++;
	      } else if ("-depth".equals(args[i])) {
	    	  info.append("爬取深度为"+Integer.parseInt(args[i+1])+"层,");
	    	  i++;
	      } else if ("-topN".equals(args[i])) {
	    	  info.append("迭代前"+Integer.parseInt(args[i+1])+"条记录");
	    	  i++;
	      }
	    }
	  return info;
  ...
  public static void main(String[] args) throws Exception {
    StringBuffer info = getInfo(args);
